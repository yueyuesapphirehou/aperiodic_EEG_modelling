function run_simulations(tau,branchNo,masterPath,oscillations)
    addpath('/home/nbrake/aperiodic_EEG_modelling/simulations/functions');
    for i = 1:30
        folder = fullfile(masterPath,['dyad_' int2str(i)]);
        local_run(tau,branchNo,folder,oscillations)
    end
end

%% local_run: function to set up model and run 10 times
function local_run(tau,branchNo,masterPath,oscillations)
    repCount = 10;
    tmax0 = 100e3;
    tmax = 10e3;
    nrnCount = 2;
