function getPSD3(masterPath)
    load('/home/nbrake/data/resources/head_models/sa_nyhead.mat');
    F2 = dir(masterPath); tempIdcs = find([F2(:).isdir]);
    F2 = F2(tempIdcs(3:end));
    P0 = [];     P1 = [];
    C0 = [];     C1 = [];
    for i = 1:length(F2)
        % if(i<6)
        %     s0 = load(fullfile(masterPath,F2(i).name,'spectra_s=0.mat'));
        %     s1 = load(fullfile(masterPath,F2(i).name,'spectra_s=1.mat'));
        % else
            [s0,s1] = analzye_simulation(fullfile(masterPath,F2(i).name),sa);
        % end
        P1(:,:,i) = s1.P(:,1:14);
        C1(:,:,i) = s1.C(1:14);
        if(~isempty(s0))
    ...
    f = s1.f;
    save(fullfile(masterPath,'analzye_simulations_10.mat'),'P0','P1','C0','C1','f')
end

function [s0,s1] = analzye_simulation(folder,sa)
    ...
    for j = 1:M
        for rep = 1:M0
            folder2 = fullfile(folder,F2(j).name,['LFPy_' int2str(rep) '_s=1']);
            ...
            q(:,:,k0*M0*(j-1)+k0*(rep-1)+1:k0*M0*(j-1)+k0*rep) = dp;
            ID(k0*M0*(j-1)+k0*(rep-1)+1:k0*M0*(j-1)+k0*rep) = j;
        end
    end
    % save(fullfile(folder,'dipoles.mat'),'q');
    % s1 = []; s0 = []; return;

    g = findgroups(ID(:)');
    [f,P,C] = expectedEEGspectra(gpuArray(q),sa,idcs);
    C =  splitapply(@(x)mean(x),C,g(2:2:end)');
    P =  splitapply(@(x)mean(x,2),P,g);
    save(fullfile(folder,'spectra_s=0.mat'),'P','f','C');
    s1.f = f;
    s1.P = P;
    s1.C = C;
    s0 = []; return;
    ...
    for j = 1:M
        for rep = 1:M0
            folder2 = fullfile(folder,F2(j).name,['LFPy_' int2str(rep) '_s=0']);
            if(~exist(folder2))
                s0 = [];
                return;
            end
    ...
    P =  splitapply(@(x)mean(x,2),P,g);
    C =  splitapply(@(x)mean(x),C,g(2:2:end)');
    save(fullfile(folder,'spectra_s=1.mat'),'P','f','C');
    s0.f = f;
    s0.P = P;
    s0.C = C;
end

function [f,P,C] = expectedEEGspectra(dipoles,sa,idcs)
    % dipoles = dipoles(4001:8e3,:,:);
    C = gpuArray(zeros(size(dipoles,3)/2,1));
    for j = 1:length(idcs)
        eeg = getEEG(dipoles,sa,idcs(j));
        for k = 1:length(C)
            C(k) = C(k) + corr(eeg(:,2*(k-1)+1),eeg(:,2*k));
        end
        temp = mypmtm(detrend(eeg,'constant'),2e3,10);
       if(j==1)
            P = zeros(size(temp));
        end
        P = P+temp;
