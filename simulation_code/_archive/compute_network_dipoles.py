...
def import_postsyanptic_network(networkPath):
    ...
    synFiles = os.listdir(synDir)
    synFiles.sort()
    synapseFiles = [synDir + '/' + x for x in synFiles]
...
def main(mFile,synapseFiles,preEI,preSpikes,savePath,T_MAX=100,activeSoma=False,propofol=0):
    ...
    for k,file in enumerate(synapseFiles):
        ...
        #Save results
        nrnID.append(str.split(file,'/')[-1][:-4])
        saveFile = savePath+'/LFPy/'+nrnID[-1] +'.csv'
    ...
    saveFile = savePath+'/LFPy/'+ nrnID[0] + '_to_' + nrnID[-1] + '.csv'
