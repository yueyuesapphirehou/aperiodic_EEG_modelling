...
for i = 1:N_in_pre
    ...
    for j = 2:k+1
        ...
        t0 = tsE(idcs).*idcs2+tmax*rand(nTrans,1).*(1-idcs);
...
for j = 1:length(M)
    ...
    elevation_E(j+N_ex_pre) = elevation_E(M(j)); % Copy synapse location
    azimuth_E(j+N_ex_pre) = azimuth_E(M(j)); % Copy synapse location

    tsE = [tsE;tsE(idcs)]; % Copy spike times
    idsE = [idsE;(j+N_ex_pre)*ones(length(idcs),1)]; % New synapse ID
end

t = truncate(makedist('Normal','mu',1,'sigma',2),-1,1)
