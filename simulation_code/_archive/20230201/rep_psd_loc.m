...
[f,P] = expectedEEGspectrum(dp,sa,idcs);
time = toc;
save('/home/nbrake/data/passive/PSD_rep.mat','f','P','time');

function [f,P] = expectedEEGspectrum(dipoles,sa,idcs)
    q = gpuArray(dipoles);
    for j = 1:length(idcs)
        temp = getEEG(q,sa,idcs(j));
        if(j==1)
            eeg = gpuArray(zeros(length(temp),length(idcs)));
        end
        eeg(:,j) = temp;
    end
    P = mypmtm(eeg,2e3,2)*pi/2;
    f = 0.5:0.5:1e3;
end
