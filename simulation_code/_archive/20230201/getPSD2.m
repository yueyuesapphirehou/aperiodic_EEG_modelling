function getPSD3(masterPath)
    ...
    C0 = [];
    for i = 1:length(F2)
        s0 = analzye_simulation(fullfile(masterPath,F2(i).name),sa);
        P0(:,:,i) = s0.P;
        C0(:,:,i) = s0.C;
    end
    f = s0.f;
    save(fullfile(masterPath,'analzye_simulations.mat'),'P0','C0','f')
end

function s0 = analzye_simulation(folder,sa)
